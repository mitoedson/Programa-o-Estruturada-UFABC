...
void separar_numeros_primos(ListaNumeros *lista) {
    ...
    // ponteiros de apoio
    int *novo_numeros = malloc(sizeof(int));
    int *novo_primos = malloc(sizeof(int));
    ...
    // muda valor de lista->n_numeros
    *novo_numeros = lista->n_numeros - contador_primos;
    lista->n_numeros = *novo_numeros;
    // realoca ponteiros lista->numeros e transfere
    lista->numeros = (int *) realloc(lista->numeros, sizeof(*novo_numeros) * sizeof(int));
    for(i = 0; i < *novo_numeros; i++)
        lista->numeros[i] = transfere_numeros[i];

    // muda valor de lista->n_primos
    *novo_primos = lista->n_primos + contador_primos;
    lista->n_primos = *novo_primos;
    // realoca ponteiros lista->primos e transfere
    lista->primos = (int *) realloc(lista->primos, *novo_primos * sizeof(int));
    j = 0;  
    for(i = (*novo_primos - contador_primos); i < *novo_primos; i++){
        lista->primos[i] = transfere_primos[j];
        j++;
    }

    // libera ponteiros de apoio
    free(novo_numeros);
    free(novo_primos);
